...
function constroiDataSet(dados,funcao){
  ...
  var dataSet = []
    ...
    if(funcao==1){
          for( j in dados){
        n=n+1
              dataSet.push([n,dados[j].cliente,new Intl.NumberFormat().format(dados[j].total)])
  ...
  if(funcao==2){
    for( j in dados){
        n=n+1
              dataSet.push([n,dados[j].artigo,dados[j].qtd,new Intl.NumberFormat().format(dados[j].total)])
  ...
  if(funcao==6){
    for( j in dados){
        n=n+1
              dataSet.push([n,me[j],new Intl.NumberFormat().format(dados[j])])
...
function grafico(descricao,quantidade,nome){
...
///alimentado a istribução mensal
 ...
 // var funcao = $("#funcao").val()
      ...
      grafico(descricao,quantidade,nome);
...
//console.log(inicio+"/"+fim+"/"+moeda)
      if(funcao==1){
        ...
        href = "{{ route('pdf.report_clientes',["moeda","inicio","fim"]) }}"
      }else if(funcao==2){
        var caminho= "{{ route('vendas.artigoGraficos',["moeda","inicio","fim"]) }}";
        href = "{{ route('pdf.report_artigosVendidos',["moeda","inicio","fim"]) }}"
      }else if(funcao==3){
        ...
        href = "{{ route('pdf.report_distribuicaoMensal',["moeda","inicio","fim"]) }}"
...
//console.log(inicio+"/"+fim+"/"+moeda)
        ...
        href = "{{ route('pdf.report_distribuicaoMensalCliente',["moeda","inicio","fim","cliente"]) }}"


       href = href.replace('moeda',moeda);
      href = href.replace('inicio',inicio);
      href = href.replace('fim',fim);
      href = href.replace('cliente',cliente)
      $("#gerarPDF").attr("href",href)
        if(!inicio){
            ano= $("#ano").val()
            if(!ano){
